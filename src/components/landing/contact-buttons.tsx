...
export function ContactButtons({ phone, address = "" }: ContactButtonsProps) {
  ...
  const actionClass = "home-contact-action flex min-h-11 min-w-0 flex-1 items-center justify-center gap-2 rounded-full border border-border/80 bg-card px-2.5 py-2.5 text-caption font-medium transition-[background-color,border-color,transform] duration-200 hover:border-foreground/20 hover:bg-muted active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60";

  return (
    <section className="px-4 pb-3" aria-labelledby="contact-heading">
      <div className="mx-auto max-w-lg">
        <h2 id="contact-heading" className="mb-3 px-1 text-small font-bold text-muted-foreground">اگر سوالی داری</h2>
