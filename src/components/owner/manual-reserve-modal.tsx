...
export function ManualReserveModal({
  ...
  useEffect(() => {
    ...
    if (!endTime || serviceChanged) {
      setEndTime(calculateEndTime(startTime, selectedService.duration_minutes));
    }
    lastResolvedServiceIdRef.current = resolvedServiceId;
  }, [endTime, selectedService, resolvedServiceId, startTime]);
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {activeServices.length > 0 ? (
            ...
            <div className="mt-1 rounded-xl border border-border bg-muted/40 px-3 py-3 text-sm text-muted-foreground">
        ...
        {submitError && <p role="alert" className="text-[12px] text-destructive text-center">{submitError}</p>}
      </div>
