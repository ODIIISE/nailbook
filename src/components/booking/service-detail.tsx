...
export function ServiceDetail({ service, popularLast30Days }: Props) {
  ...
  return (
    <div className="space-y-3">
      {hasImage ? (
        <div className="relative w-full aspect-[16/10] overflow-hidden rounded-2xl border border-border bg-muted">
          ...
          <div className="absolute bottom-0 inset-x-0 p-3 bg-gradient-to-t from-black/60 to-transparent">
            <h1 className="text-[20px] font-extrabold text-white leading-tight">{service.name}</h1>
            <div className="flex items-center gap-2 mt-1 text-[13px] text-white/80">
        ...
        <div className="p-4 rounded-2xl border border-border bg-card">
          <h1 className="text-[20px] font-extrabold text-foreground leading-tight">{service.name}</h1>
      ...
      {description && (
        <p className="text-[13px] leading-7 text-muted-foreground px-1">{description}</p>
      )}
      ...
      {bestFor.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center gap-2 px-1">
            ...
            <span className="text-[13px] font-bold text-foreground">این خدمت برای چه کسانی مناسب است؟</span>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {bestFor.map((tag, idx) => (
              <span
                key={`${tag}-${idx}`}
                className="px-3 py-1 rounded-full bg-card border border-border text-[12px] text-foreground"
              >
                {tag}
              </span>
            ))}
